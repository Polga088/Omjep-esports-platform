...
export default function StoreManagement() {
  ...
  const [selectedUser, setSelectedUser] = useState<AdminUserSearchHit | null>(null);
  const [userSearchQuery, setUserSearchQuery] = useState('');
  const [debouncedUserQuery, setDebouncedUserQuery] = useState('');
  const [userSearchResults, setUserSearchResults] = useState<AdminUserSearchHit[]>([]);
  const [userSearchLoading, setUserSearchLoading] = useState(false);
  const [userSearchOpen, setUserSearchOpen] = useState(false);
  const grantUserPickerRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const t = window.setTimeout(() => setDebouncedUserQuery(userSearchQuery.trim()), 300);
  ...
  useEffect(() => {
    ...
    void api
      .get<AdminUserSearchHit[]>('/admin/users/search', { params: { query: debouncedUserQuery } })
      .then((res) => {
        if (!cancelled) setUserSearchResults(Array.isArray(res.data) ? res.data : []);
      })
      .catch(() => {
  ...
  useEffect(() => {
    const onDown = (e: MouseEvent) => {
      const el = grantUserPickerRef.current;
  ...
  const pickGrantUser = (hit: AdminUserSearchHit) => {
    setSelectedUserId(hit.id);
    setSelectedUser(hit);
    setUserSearchQuery('');
    setUserSearchResults([]);
    setUserSearchOpen(false);
  ...
  const submitGrant = async () => {
    const uuid = selectedUserId.trim();
    const uuidOk =
    ...
    <div className="space-y-8 max-w-6xl">
      ...
      <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-5 backdrop-blur-md">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="relative sm:col-span-2" ref={grantUserPickerRef}>
            <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500">
              Joueur
            </label>
            {selectedUser ? (
              <div className="flex items-center gap-3 rounded-lg border border-emerald-500/30 bg-black/30 px-3 py-2">
                {selectedUser.avatar ? (
                  <img
                  ...
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-700/80 text-xs font-bold text-slate-300">
                    {selectedUser.username.slice(0, 2).toUpperCase()}
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-white">{selectedUser.username}</p>
                  <p className="truncate font-mono text-[10px] text-slate-500">{selectedUser.id}</p>
                </div>
                <button
                  type="button"
                  onClick={() => clearGrantUserSelection()}
                  className="shrink-0 rounded-md px-2 py-1 text-[11px] font-semibold text-amber-400/90 hover:bg-white/5 hover:text-amber-300"
                >
                  Changer
                </button>
              </div>
              ...
              <>
                <input
                  ...
                  autoComplete="off"
                  className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm text-white placeholder:text-slate-600 focus:border-emerald-500/40 focus:outline-none"
                />
                {userSearchOpen && (userSearchQuery.trim().length >= 2 || userSearchLoading) && (
                  <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-56 overflow-auto rounded-lg border border-white/10 bg-zinc-950 py-1 shadow-xl shadow-black/50">
                    {userSearchLoading && userSearchResults.length === 0 ? (
                      ...
                      <p className="px-3 py-2 text-xs text-slate-500">Aucun résultat.</p>
                    ) : (
                      userSearchResults.map((hit) => (
                        <button
                          key={hit.id}
                          type="button"
                          onClick={() => pickGrantUser(hit)}
                        ...
                        >
                          {hit.avatar ? (
                            ...
                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-700/80 text-[10px] font-bold text-slate-300">
                              {hit.username.slice(0, 2).toUpperCase()}
                            </div>
                          )}
                          <span className="truncate font-medium text-white">{hit.username}</span>
                        </button>
                      ))
                    )}
                  </div>
